...
public interface ShowtimeRepository extends JpaRepository<ShowtimeEntity, Long> {
    ...
    @Query("SELECT s FROM ShowtimeEntity s WHERE DATE(s.startingTime) = :date AND s.room.cinema.region = :region")
    List<ShowtimeEntity> findByShowDateAndRegion(@Param("date") LocalDate date,  @Param("region") RegionEntity region);
}
